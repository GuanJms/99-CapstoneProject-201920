...
def main():
    ...
    guitar_width_input_frame = guitar_width(main_frame, mqtt_sender)
    last_button = None
    frobot = shared_gui_delegate_on_robot.frobot()
    note_frame = music_buttons(main_frame, mqtt_sender,frobot)
...
def music_buttons(main_frame, mqtt_sender,frobot):
    ...
    for i in range(3, 6, 1):
        for j in range(len(lista)):
            ...
            if button_name != 'C3' and button_name != 'D3' and button_name != 'A5' and button_name != 'B5':
                position_x = i - 3
                position_y = j
                button_construct(position_x, position_y, music_button_frame, mqtt_sender, button_name,frobot)
    return music_button_frame


def button_construct(position_x, position_y, music_button_frame, mqtt_sender, button_name, frobot):
    button = ttk.Button(music_button_frame, text=button_name)
    button.grid(row=position_y, column=position_x)
    button["command"] = lambda: handle_note(button_name, mqtt_sender, frobot)
    button.grid(row=position_y, column=position_x)


def handle_note(button_name, mqtt_sender, frobot):
...
def handle_start_generate(k, inches, mqtt_sender):
